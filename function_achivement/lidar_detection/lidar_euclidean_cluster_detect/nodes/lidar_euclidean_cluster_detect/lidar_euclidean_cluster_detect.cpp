...
#include "autoware_config_msgs/ConfigClustering.h"
...
static void config_callback(const autoware_config_msgs::ConfigClustering::ConstPtr& input)
{
  _leaf_size = input->leaf_size;
  _clustering_distance = input->clustering_distance;
  _cluster_size_min = input->cluster_size_minimum;
  _cluster_size_max = input->cluster_size_maximum;
}
...
int main(int argc, char **argv)
{
  ...
  if (_use_multiple_thres)
  {
    ...
    std::stringstream ss_distances(str_distances.substr(1, str_distances.length() - 2));
    std::string distance;
    while (std::getline(ss_distances, distance, ',')) {
        distances.push_back(std::stod(distance));
    }
    // 解析范围参数
    std::vector<double> ranges;
    std::stringstream ss_ranges(str_ranges.substr(1, str_ranges.length() - 2));
    std::string range;
    while (std::getline(ss_ranges, range, ',')) {
        ranges.push_back(std::stod(range));
    }
    // 检查参数的有效性
    if (distances.empty() || ranges.empty()) {
        std::cerr << "Invalid size of clustering_ranges or/and clustering_distances. The size of clustering distance and clustering_ranges should not be 0." << std::endl;
        return 1;
    }
    if (distances.size() - ranges.size() != 1) {
        std::cerr << "Invalid size of clustering_ranges or/and clustering_distances. Expecting that (distances_size - ranges_size) == 1." << std::endl;
        return 1;
    }
    // 存储参数值到 _clustering_distances 和 _clustering_ranges
    _clustering_distances.assign(distances.begin(), distances.end());
    _clustering_ranges.assign(ranges.begin(), ranges.end());
    // -----------
  }
  ...
  ros::Subscriber config_sub = h.subscribe("config/clustering", 1, config_callback);
