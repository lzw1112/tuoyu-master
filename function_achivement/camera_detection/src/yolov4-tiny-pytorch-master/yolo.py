...
class YOLO(object):
    ...
    #   检测图片
    #---------------------------------------------------#
    def detect_image(self, image, nms=0.3):
        ...
        with torch.no_grad():
            ...
            results = self.bbox_util.non_max_suppression(torch.cat(outputs, 1), self.num_classes, self.input_shape, 
                        image_shape, self.letterbox_image, conf_thres = self.confidence, nms_thres = nms)
        ...
        for i, c in list(enumerate(top_label)):
            ...
            if predicted_class == 'car' or predicted_class == 'person':
                box             = top_boxes[i]
                score           = top_conf[i]
