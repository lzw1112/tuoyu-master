...
#include "autoware_config_msgs/ConfigCameraDetection.h"
...
bool QNode::init() {
  ...
  // 功能调参话题发布
  config_canny_pub = n.advertise<autoware_config_msgs::ConfigCanny>("/config/canny", 10);
  config_ROI_pub = n.advertise<autoware_config_msgs::ConfigROI>("/config/ROI", 10);
  config_hough_pub = n.advertise<autoware_config_msgs::ConfigHough>("/config/hough", 10);
  config_detection_pub = n.advertise<autoware_config_msgs::ConfigCameraDetection>("/config_detection", 10);
  // 发布频率
  ros::Rate rate(1); 
...
void QNode::publishe_config_canny(const int max_thresh, int min_thresh)
{
  autoware_config_msgs::ConfigCanny config_msgs;
  config_msgs.max_thresh = max_thresh;
  config_msgs.min_thresh = min_thresh;
  config_canny_pub.publish(config_msgs);
}



void QNode::publishe_config_hough(const int max_thresh, const int min_thresh, const int hough_thresh)
{
  autoware_config_msgs::ConfigHough config_msgs;
  config_msgs.max_thresh = max_thresh;
  config_msgs.min_thresh = min_thresh;
  config_msgs.hough_thresh = hough_thresh;
  config_hough_pub.publish(config_msgs);
}

void QNode::publishe_config_ROI(const int x, const int y, const int w, const int h)
{
  autoware_config_msgs::ConfigROI config_msgs;
  config_msgs.x = x;
  config_msgs.y = y;
  config_msgs.width = w;
  config_msgs.height = h;
  config_ROI_pub.publish(config_msgs);
}

void QNode::publishe_config_detection(const std::string data_source, const std::string detect_type, const float iou, const float confidence, const std::string cuda)
{
  autoware_config_msgs::ConfigCameraDetection config_msgs;
  config_msgs.nms = iou;
  config_msgs.confidence = confidence;
  config_msgs.cuda_detected = false;
  if(cuda == "开启")
    config_msgs.cuda_detected = true;
  config_msgs.type = detect_type;
  config_msgs.data_source = data_source;
  config_detection_pub.publish(config_msgs);
}
