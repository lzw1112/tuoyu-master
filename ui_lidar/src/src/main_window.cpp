...
void MainWindow::initUis()
{
    ...
    path_Mapmap_yaml = "/home/rise/tuoyu_ws/src/tuoyu/driver/lslidar_c16/lslidar_c16_decoder/params/decoder.yaml";
...
//
	...
	// std::ofstream ofs(path_Mapmap_yaml.c_str(), std::ios::app);
	// ofs << config;
	// ofs.close();
    ...
    button_switch_downsample = new SwitchButton;
    button_switch_ground = new SwitchButton;
    button_switch_detect = new SwitchButton;
    ...
    button_switch_downsample->setBgColorOn(green);
    button_switch_downsample->setBgColorOff(block);

    button_switch_detect->setBgColorOn(green);
    button_switch_detect->setBgColorOff(block);

    button_switch_ground->setBgColorOn(green);
    button_switch_ground->setBgColorOff(block);
    ...
    ui.horizontalLayout_downsample->insertWidget(1, button_switch_downsample);
    ui.horizontalLayout_ground->insertWidget(1, button_switch_ground);
    ui.horizontalLayout_detect->insertWidget(1, button_switch_detect);
    ...
    connect(button_switch_downsample, SIGNAL(checkedChanged(bool)),this,SLOT(slot_startUp_downsample(bool)));
    connect(button_switch_detect, SIGNAL(checkedChanged(bool)),this,SLOT(slot_startUp_detect(bool)));
    connect(button_switch_ground, SIGNAL(checkedChanged(bool)),this,SLOT(slot_startUp_ground(bool)));
...
void MainWindow::slot_set_laser_config(bool checked)
{
    ...
    std::string path_Mapmap_yaml = "/home/rise/tuoyu_ws/src/tuoyu/driver/lslidar_c16/lslidar_c16_decoder/params/decoder.yaml";
...
void MainWindow::slot_update_ros_sense_data(SENSE_STATE sense_state)
{
        ...
        {
            ui.label_laser_state->setPalette(pa);
            ui.label_laser_state->setStyleSheet("color:green;");
            ui.label_laser_state->setText("在线");
        }
        else
        {
            ui.label_laser_state->setPalette(pa2);
            ui.label_laser_state->setStyleSheet("color:red;");
            ui.label_laser_state->setText("离线"); 
        }

        if (sense_state.laser2_state == 1)
        {
           ui.label_laser2_state->setPalette(pa);
           ui.label_laser2_state->setStyleSheet("color:green;");
           ui.label_laser2_state->setText("在线"); 
        }
        else
        {
           ui.label_laser2_state->setPalette(pa2);
           ui.label_laser2_state->setStyleSheet("color:red;");
           ui.label_laser2_state->setText("离线"); 
        }
}
...
void MainWindow::slot_startUp_downsample(bool checked)
{
    char yaml_get[200];
    if (1 == checked)
    {
        system("rosnode kill /voxel_grid_filter ");
        sprintf(yaml_get, "roslaunch points_downsampler points_downsample.launch &");
        printf("-------------- %s %s ----------\n", __func__, yaml_get);
        system(yaml_get);
    }
    else
...
void MainWindow::slot_startUp_detect(bool checked)
{
    char yaml_get[200];
    if (1 == checked)
    {
        system("rosnode kill /lidar_euclidean_cluster_detect ");
        sprintf(yaml_get, "roslaunch lidar_euclidean_cluster_detect lidar_euclidean_cluster_detect.launch &");
        printf("-------------- %s %s ----------\n", __func__, yaml_get);
        system(yaml_get);
    }
    else
...
void MainWindow::slot_startUp_ground(bool checked)
{
    char yaml_get[200];
    if (1 == checked)
    {
        system("rosnode kill /ray_ground_filter ");
        sprintf(yaml_get, "roslaunch points_preprocessor ray_ground_filter.launch &");
        printf("-------------- %s %s ----------\n", __func__, yaml_get);
        system(yaml_get);
    }
    else
...
MainWindow::~MainWindow() {
  ...
  sprintf(yaml_get,
    ...
    delete  button_switch_downsample;
    delete  button_switch_detect;
    delete  button_switch_ground;
...
void MainWindow::on_button_connect_clicked(bool check ) {
	...
	if ( ui.checkbox_use_environment->isChecked() ) {
        if ( !qnode.init() )
        {
            ...
            ui.label_laser_state->setStyleSheet("color:red;");
            ui.label_laser2_state->setStyleSheet("color:red;");
            ui.label_tof_state->setStyleSheet("color:red;");
            ui.label_tof_state->setText("离线");
            ui.label_laser2_state->setText("离线");
            ui.label_laser_state->setText("离线");
